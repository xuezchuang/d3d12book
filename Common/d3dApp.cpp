...
bool D3DApp::InitMainWindow()
{
	...
	AdjustWindowRect(&R, dwWndStyle, FALSE);
	//AdjustWindowRect(&R, WS_OVERLAPPEDWINDOW, false);
	int width = R.right - R.left;
	int height = R.bottom - R.top;

	mhMainWnd = CreateWindow(L"MainWnd", mMainWndCaption.c_str(),
							 dwWndStyle, CW_USEDEFAULT, CW_USEDEFAULT, width, height, 0, 0, mhAppInst, 0);
	if(!mhMainWnd)
	{
...
//Render Target View And Depth test View
...
void D3DApp::FlushCommandQueue()
{
	...
	if(gpufence < mCurrentFence + 1)
	{
		...
		//
		//UINT64 ntest11 = mTimer.getcurtime();
		//// Wait until the GPU hits current fence event is fired.
		WaitForSingleObject(eventHandle, INFINITE);
		////::MsgWaitForMultipleObjects(1, &eventHandle, FALSE, INFINITE, QS_ALLINPUT);

		CloseHandle(eventHandle);
		//UINT64 ntest22 = mTimer.getcurtime();

		wstring fpsStr = to_wstring(mCurrentFence) + L" " + to_wstring(gpufence) + L"\n";
		//wstring CountStr = to_wstring(mtest);
		//wstring windowText = 
			//L" : " + fpsStr +
		//	L"  Ö´ÐÐ: " + CountStr + L"\n";
...
void D3DApp::CalculateFrameStats()
{
	...
	frameCnt++;
	mSwapChain->GetFrameStatistics(&stats);//) && (stats.PresentCount > LastRetrievedID))
